...
class IonSidebarPanel(tk.Toplevel):
    def __init__(self, parent, shot_ions_dict, on_update, *args, **kwargs):
        ...
        for ions in shot_ions_dict.values():
            for ion_label, wl, intens in ions:
                if ion_label not in all_ions_dict:
                    all_ions_dict[ion_label] = wl
                    
        all_ions = sorted(all_ions_dict.keys(), key=lambda k: all_ions_dict[k])
