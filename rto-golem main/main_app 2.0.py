...
import subprocess
import pyperclip
import spectrometry_analyzer_prueba
import json
import pickle
import h5py
...
class IonSidebarPanel(tk.Toplevel):
    def __init__(self, parent, shot_ions_dict, on_update, *args, **kwargs):
        ...
        tk.Button(frame, text="Update Plot", command=self.on_update)\
...
class FrameViewerPanel(tk.Toplevel):
    def __init__(self, parent, shot_number, h5_path, *args, **kwargs):
        ...
        # 1. Leer los datos H5 directamente
        ...
        # 2. Configurar la figura de Matplotlib integrada a Tkinter
        ...
        self.ax.set_xlabel('Wavelength (Å)', fontsize=9)
        self.ax.set_ylabel('Intensity (A.U.)', fontsize=9)
        self.ax.set_title(f'Frame 0 | Time: 0.0 - 2.0 ms', fontsize=10)
        self.ax.grid(True, linestyle='--', alpha=0.6)
        self.fig.tight_layout()

        # 3. Control Deslizante (Slider) nativo de Tkinter
    ...
    def actualizar_frame(self, val):
        ...
        t_inicio = frame_actual * 2.0
        t_fin = (frame_actual + 1) * 2.0
        self.ax.set_title(f'Frame {frame_actual} | Tiempo: {t_inicio:.1f} - {t_fin:.1f} ms', fontsize=10)
...
class TokamakDataViewer:
    def __init__(self, root):
        ...
        try:
            self.nist_df = spectrometry_analyzer_prueba.load_nist("nist_spectral_lines.csv")
            if self.nist_df is None:
                messagebox.showwarning("Warning", "Could not load 'nist_spectral_lines.csv'.\nSpectrometry analysis will not work.")
    ...
    def load_shot(self):
        ...
        try:
            ...
            # Load fast camera data (protegido contra errores 404)
            try:
            ...
            h5_file_path = spectrometry_analyzer_prueba.download_h5(shot_number)
            ...
            if h5_file_path and self.nist_df is not None:
                ions, wls, intens = spectrometry_analyzer_prueba._detect_main_ions_for_panel(
            ...
            shot_data = {
                ...
                'end_time': self.find_plasma_end_time(ip_data, threshold=0.05),
    ...
    def show_ion_sidebar(self):
        if not self.shot_ions_for_panel:
            messagebox.showwarning("No data", "Please load a shot first to access the panel.")
            return
    ...
    def plot_data(self):
        ...
        for shot, data in self.shots.items():
            ...
            if ions_scales_dict:
                spectrometry_analyzer_prueba.plot_ion_evolution_on_ax(
                    ax=ax_spec,
                    shot_number=shot,
                    shot_color=color,
    ...
    def find_plasma_end_time(self, ip_data, threshold=0.05):
        if ip_data.empty or 'Ip' not in ip_data.columns:
            return float('inf')
        ip_values = ip_data['Ip'].values
        time_values = ip_data['time_ms'].values
        ...
        above_threshold = np.where(ip_values > threshold_value)[0]
        if len(above_threshold) > 0:
            return time_values[above_threshold[-1]]
        return float('inf')
